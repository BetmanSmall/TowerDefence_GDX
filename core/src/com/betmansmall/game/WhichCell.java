...
public class WhichCell {
    ...
    private static float getDelta(float q,float w, float e, float r, GridPoint2 touch) {
    ...
    private static int correlation(float q, float w, float e, float r, GridPoint2 touch) {
    ...
    private static int getOktant(float X, float Y) {
        if(0 <= Y && Y < X)
            return 1;
        else if(0 < X && X <= Y)
            return 2;
    ...
    private static float opredelitel(float x, float s, float t, float f) {
        return x*f - s*t;
    }
}
